...
export const getMail =  async (req, res) => {
    try {
        const [result] = await pool.query('SELECT * FROM mail WHERE idmail = ?', [req.params.id]);
        if (result === 0){
...
export const updateMail = async (req, res) => {
    try {
        const result = await pool.query('UPDATE mail SET ? WHERE idmail = ?', [req.body, req.params.id]);
        res.json(result);
...
export const deleteMail = async (req, res) => {
    try {
        const  [result] = await pool.query('DELETE FROM mail WHERE idmail = ?', [req.params.id]);
